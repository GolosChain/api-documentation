...
| sequenceKey | string |                Нет                 |   null    |                          | Идентификатор пагинации, необходимо указать для того чтобы получить следующий слепок данных, должен быть равен значению что вернул сервер на предыдущий слепок. Для получения первого слепка параметр должен отсутствовать либо быть равным null. |
